...
class TicTacToeGame:
    ...
    def getWinner(self) -> Optional[str]:
        ...
        for i1, i2, i3 in winning_combinations:
            if self.state[i1] == self.state[i2] == self.state[i3] != self.EMPTY:
                return self.state[i1]
        return None
    
    def isGameOver(self) -> bool:
        return (self.getWinner() is not None) or (self.EMPTY not in self.state) # Check for winner or draw
    ...
    def get_possible_actions(self) -> list[int]:
        return [i for i, cell in enumerate(self.state) if cell == self.EMPTY]
...
if __name__ == '__main__':
    game = TicTacToeGame()
    game.print_state()
    game.save_q_table({game.get_state_key(): {0: 1.0}})
